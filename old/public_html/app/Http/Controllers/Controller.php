...
function detailviewsave( $model ,$request , $detail , $id )

	{
		...
		if($access['is_add'] == '1' && $access['is_edit'] == '1' )
		{
			...
			if(isset($request['counter']) && $request['counter']!="")
            {
                $total=count($request['counter']);
            } else $total = 0;
			...
			$mkeys = array();
			$getAllCurrentData = \DB::table($detail['table'])->where( $detail['key'], $id  )->get();
			// modificato $detail['master_key']
