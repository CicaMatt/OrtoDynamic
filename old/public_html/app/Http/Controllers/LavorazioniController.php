...
use App\Models\Lavorazioni;
use Illuminate\Http\Request;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;
use Validator, Input, Redirect ; 


class LavorazioniController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'lavorazioni';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Lavorazioni();	
		$this->modelview = new  \App\Models\Prodottilavorazioni();
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'lavorazioni',
	...
	public function destroy( $request)
	{
		...
		if(count($request->input('ids')) >=1)
		{
			...
			\DB::table('item_lavorazioni')->whereIn('id_lavorazione',$request->input('ids'))->delete();
	...
	public static function display(  )
	{
		...
		$model  = new Lavorazioni();
		$info = $model::makeInfo('lavorazioni');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('lavorazioni.public.view',$data);			
			}			
		} 
		else {
			...
			return view('lavorazioni.public.index',$data);	
		}
