...
class ClientiController extends Controller {
	...
	public $module = 'clienti';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Clienti();	
		...
		switch ($task)
		{
			default:
				...
				if ($validator->passes()) 
				{
                    ...
                    elseif(!is_null($request->input('both')))  {
                        $idPreventivo = \DB::table('preventivi')->insertGetId(['id_cliente' => $id]);
				        return redirect('preventivi/' . $idPreventivo . '/edit?return=');                    
                    }
			...
			case 'inserisciPreventivo':
				$id = $this->inserisciPreventivo( $request );
				return redirect('preventivi/' . $id . '/edit?return=');
				break;
		}	
	...
	public static function display(  )
	{
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('clienti.public.view',$data);			
			}			
		} 
		else {
			...
			return view('clienti.public.index',$data);	
		}
	...
	public function inserisciPreventivo( $request)
	{
	    $idCliente = $request->input('id_cliente');
	    if(!\Auth::check()) 
			return redirect('user/login')->with('status', 'error')->with('message','You are not login');
		$id = \DB::table('preventivi')->insertGetId(['id_cliente' => $idCliente]);
        return $id;
	}
}
