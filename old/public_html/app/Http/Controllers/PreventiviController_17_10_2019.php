		...
		if(count($request->input('ids')) >=1)
		{
		    ...
		    if (count($idState)>=1) {
        	    ...
        	    if ($newState == 'IN LAVORAZIONE' || $newState == 'IN LAVORAZIONE SENZA AUTORIZZAZIONE')
        	    {
                    ...
                    $items = \DB::table('item_preventivi')->where('id_preventivo', (int)$request->input('ids'))->get();
                    
                    foreach ($items as $item ) {
                       $idItem = \DB::table('item_lavorazioni')->insertGetId(['id_item_preventivi' => $item->id,'codice_nomenclatore' => $item->codice_nomenclatore,'quantita'=>$item->quantita,'id_lavorazione' => $id,'stato'=>'IN LAVORAZIONE']);
                    }
