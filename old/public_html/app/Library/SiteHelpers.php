...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
		...
		switch($type)
		{
			...
			case 'select';
				...
				if($option['opt_type'] =='external')
				{
					...
					$data = DB::table($option['lookup_table']);

                    if ($option['lookup_table'] == 'nomenclatore' && empty($value)) {
                        $data = $data->where('anno', '2025');
                    }
	...
	public static function formatRows( $value ,$attr , $row = null )
	{
		...
		}elseif($format_as =='percentuale') {
		    if ($value != '') $value = $value . ' %';
        
		}elseif($format_as =='euro') {
		    if ($value != '') $value = $value . ' €';
