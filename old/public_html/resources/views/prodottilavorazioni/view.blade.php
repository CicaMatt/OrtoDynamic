	...
	<div class="sbox">
		<div class="sbox-title clearfix">
			<div class="sbox-tools pull-left" >
		   		<a href="{{ ($prevnext['prev'] != '' ? url('prodottilavorazioni/'.$prevnext['prev'].'?return='.$return ) : '#') }}" class="tips btn btn-sm"><i class="fa fa-arrow-left"></i>  </a>	
				<a href="{{ ($prevnext['next'] != '' ? url('prodottilavorazioni/'.$prevnext['next'].'?return='.$return ) : '#') }}" class="tips btn btn-sm "> <i class="fa fa-arrow-right"></i>  </a>					
			</div>	

			<div class="sbox-tools" >
				@if($access['is_add'] ==1)
		   		<a href="{{ url('prodottilavorazioni/'.$id.'/edit?return='.$return) }}" class="tips btn btn-sm  " title="{{ __('core.btn_edit') }}"><i class="fa  fa-pencil"></i></a>
				@endif
				<a href="{{ url('prodottilavorazioni?return='.$return) }}" class="tips btn btn-sm  " title="{{ __('core.btn_back') }}"><i class="fa  fa-times"></i></a>		
			</div>
		</div>
			...
			<div class="table-responsive">
				<table class="table table-striped " >
					...
					<tr>
						...
						<td>{{ date('',strtotime($row->data_annullamento)) }} </td>
					...
					<tr>
						...
						<td>{{ date('',strtotime($row->data_ordine)) }} </td>
					...
					<tr>
						...
						<td>{{ date('',strtotime($row->data_consegna_parziale)) }} </td>
					...
					<tr>
						...
						<td>{{ date('',strtotime($row->data_consegna)) }} </td>
