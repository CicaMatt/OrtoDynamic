		    ...
		    <table class="table table-striped table-hover " id="{{ $pageModule }}Table">
		        ...
		        <tbody>        						
		            @foreach ($rowData as $row)
		                <tr>
							<td > {{ ++$i }} </td>
							<td ><input type="checkbox" class="ids" name="ids[]" value="{{ $row->group_id }}" />  </td>
							<td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li><a href="{{ url('core/groups/'.$row->group_id.'?return='.$return)}}" class="tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li><a  href="{{ url('core/groups/'.$row->group_id.'/edit?return='.$return) }}" class="tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
