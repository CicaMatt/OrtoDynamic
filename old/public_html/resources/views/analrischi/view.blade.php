	...
	<div class="sbox">
		<div class="sbox-title clearfix">
			<div class="sbox-tools pull-left" >
		   		<a href="{{ ($prevnext['prev'] != '' ? url('analrischi/'.$prevnext['prev'].'?return='.$return ) : '#') }}" class="tips btn btn-sm"><i class="fa fa-arrow-left"></i>  </a>	
				<a href="{{ ($prevnext['next'] != '' ? url('analrischi/'.$prevnext['next'].'?return='.$return ) : '#') }}" class="tips btn btn-sm "> <i class="fa fa-arrow-right"></i>  </a>					
			</div>	

			<div class="sbox-tools" >
				@if($access['is_add'] ==1)
		   		<a href="{{ url('analrischi/'.$id.'/edit?return='.$return) }}" class="tips btn btn-sm  " title="{{ __('core.btn_edit') }}"><i class="fa  fa-pencil"></i></a>
				@endif
				<a href="{{ url('analrischi?return='.$return) }}" class="tips btn btn-sm  " title="{{ __('core.btn_back') }}"><i class="fa  fa-times"></i></a>		
			</div>
		</div>
		<div class="sbox-content">
			<div class="table-responsive">
				<table class="table table-striped " >
					...
					<tr>
						<td width='30%' class='label-view text-right'>{{ SiteHelpers::activeLang('Riutilizzo Dispositivo', (isset($fields['riutilizzo_dispositivo']['language'])? $fields['riutilizzo_dispositivo']['language'] : array())) }}</td>
						<td>{{ $row->riutilizzo_dispositivo}} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'>{{ SiteHelpers::activeLang('Composizione Chimica Materiali', (isset($fields['composizione_chimica_materiali']['language'])? $fields['composizione_chimica_materiali']['language'] : array())) }}</td>
						<td>{{ $row->composizione_chimica_materiali}} </td>
