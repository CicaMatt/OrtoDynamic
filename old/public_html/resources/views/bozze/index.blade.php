	...
	<div class="page-content-wrapper no-margin">
		...
		<div class="sbox">
			...
			<div class="row">
				<div class="col-md-4"> 	
					@if($access['is_add'] ==1)
					<a href="{{ url('bozze/create?return='.$return) }}" class="btn btn-default btn-sm"  
			...
			<div class="table-responsive" style="padding-bottom: 70px;">
 			{!! Form::open(array('url'=>'bozze?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
							...
							<td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li><a href="{{ url('bozze/'.$row->id.'?return='.$return)}}" class="tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
         

<li><a target="_blank" href="{{ url('generaPdf.php?checkedvalue='.$row->id.'&tipologia=scheda')}}" class="tips" title="Genera Scheda PDF"> Genera Scheda bozza </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li><a  href="{{ url('bozze/'.$row->id.'/edit?return='.$return) }}" class="tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
